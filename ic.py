        ...
        with Bar('Getting IC Component Metrics..', max=len(self.components)) as bar:
            for component in self.components:
                issues = self.jira.search_issues(f'project = HELPDESK AND status in (Resolved, Closed) AND assignee = {self.username} AND \
                                                resolutiondate >= -{offset}d and component in("{component.name}")')
                if component.name == 'Fedex' and len(issues):
                    self.ic_component_metrics[component.name] = {'total_issues':len(issues), 'time': 0, 'inbound_len': 0, 'outbound_len': 0, 'cost_avg': 0}
                    for x in issues:
                        if x.fields.customfield_16736 != None:
                            ...
                            try:
                                ...
                                cost = float(self.fedex.estimate(sender_zip, recipient_zip, 5, 1000, 'FEDEX_2_Day')['totalNetCharge'])
                        ...
                        self.ic_component_metrics[component.name]['time'] += date_delta.days
                        self.ic_component_metrics[component.name]['cost_avg'] += cost

                    self.ic_component_metrics[component.name]['time'] /= len(issues)
                    self.ic_component_metrics[component.name]['cost_avg'] /= self.ic_component_metrics[component.name]['total_issues']
                elif len(issues):
